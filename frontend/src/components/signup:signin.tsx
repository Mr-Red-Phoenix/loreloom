...
interface BlurFadeProps { children: React.ReactNode; className?: string; variant?: { hidden: { y: number; opacity: number; filter: string }; visible: { y: number; opacity: number; filter: string } }; duration?: number; delay?: number; yOffset?: number; inView?: boolean; inViewMargin?: string; blur?: string; }
function BlurFade({ children, className, variant, duration = 0.4, delay = 0, yOffset = 6, inView = true, inViewMargin = "-50px", blur = "6px" }: BlurFadeProps) {
  ...
  return (
    <motion.div ref={ref} initial="hidden" animate={isInView ? "visible" : "hidden"} exit="hidden" variants={combinedVariants as any} transition={{ delay: 0.04 + delay, duration, ease: "easeOut" }} className={className}>
      {children}
    </motion.div>
  );
}
...
const GradientBackground = () => (
    <>
        <style>
            {` @keyframes float1 { 0% { transform: translate(0, 0); } 50% { transform: translate(-20px, 20px); } 100% { transform: translate(0, 0); } } @keyframes float2 { 0% { transform: translate(0, 0); } 50% { transform: translate(20px, -20px); } 100% { transform: translate(0, 0); } } `}
        </style>
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full opacity-80 pointer-events-none">
            <defs>
                <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: '#d4a373', stopOpacity:0.9}} /><stop offset="100%" style={{stopColor: '#a16207', stopOpacity:0.6}} /></linearGradient>
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: '#a855f7', stopOpacity:0.8}} /><stop offset="50%" style={{stopColor: '#6b21a8', stopOpacity:0.6}} /><stop offset="100%" style={{stopColor: '#3b0764', stopOpacity:0.4}} /></linearGradient>
                <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%"><stop offset="0%" style={{stopColor: '#be123c', stopOpacity:0.7}} /><stop offset="100%" style={{stopColor: '#4c0519', stopOpacity:0.2}} /></radialGradient>
                <filter id="rev_blur1" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="80"/></filter>
                <filter id="rev_blur2" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="90"/></filter>
                <filter id="rev_blur3" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="100"/></filter>
            </defs>
            <g style={{ animation: 'float1 20s ease-in-out infinite' }}>
                <ellipse cx="150" cy="550" rx="400" ry="300" fill="url(#rev_grad1)" filter="url(#rev_blur1)" transform="rotate(-20 150 550)"/>
                <ellipse cx="650" cy="100" rx="350" ry="250" fill="url(#rev_grad2)" filter="url(#rev_blur2)" transform="rotate(15 650 100)"/>
            </g>
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                <circle cx="750" cy="550" r="300" fill="url(#rev_grad3)" filter="url(#rev_blur3)" opacity="0.8"/>
            </g>
        </svg>
    </>
);
...
const loginModalSteps = [
    ...
    { message: "Just a moment...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", mode = "signup", onLogin, onSignUp, onGoogleLogin, onGitHubLogin }: AuthComponentProps) => {
  ...
  const steps = mode === "login" ? loginModalSteps : modalSteps;
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    const callback = mode === "login" ? onLogin : onSignUp;
    if (callback) {
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        if (isEmailValid) {
          if (mode === "login") setAuthStep("password");
          else setAuthStep("password");
        }
    } else if (authStep === 'password') {
        if (isPasswordValid) {
          if (mode === "login") handleFinalSubmit(new Event('submit') as any);
                ...
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton contentClassName="flex items-center justify-center gap-2" size="sm" onClick={() => onGoogleLogin?.()}><GoogleIcon /><span className="font-semibold text-foreground">Google</span></GlassButton>
                            <GlassButton contentClassName="flex items-center justify-center gap-2" size="sm" onClick={() => onGitHubLogin?.()}><GitHubIcon /><span className="font-semibold text-foreground">GitHub</span></GlassButton>
                        </div></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{mode === "login" ? "Enter your password" : "Create your password"}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{mode === "login" ? "Use the password you signed up with." : "Your password must be at least 6 characters long."}</p></BlurFade>
                    </motion.div>}
